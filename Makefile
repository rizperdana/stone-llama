...
# linux/amd64 is the only *supported* platform; the other three are published
# but compiled-only (see docs/RELEASE.md for the honest platform matrix).
RELEASE_TARGETS := linux/amd64 windows/amd64 darwin/amd64 darwin/arm64
...
dist: clean ## Cross-compile release artifacts into dist/
	...
	for tgt in $(RELEASE_TARGETS); do \
		...
		if [ "$$ext" = "zip" ]; then \
			...
			tar czf dist/$$name.tar.gz $$stagedir; \
		fi; \
		(cd dist && $(SHA256) $$name.$$ext > $$name.$$ext.sha256); \
		sz=$$(wc -c < dist/$$name.$$ext | tr -d ' '); \
		human=$$(du -h dist/$$name.$$ext | cut -f1); \
		echo "  → dist/$$name.$$ext ($$human, $$sz bytes)"; \
		rm -rf $$stagedir; \
	done; \
(cd dist && for f in stone-llama_*.tar.gz stone-llama_*.zip; do [ -f "$$f" ] && $(SHA256) "$$f" || true; done > checksums.txt); \
