...
**Dependencies:** `gopkg.in/yaml.v3` (emit TabbyAPI config) and nothing else. HTTP, SSE, JSON, progress rendering: stdlib.
...
- `THIRD-PARTY.md`: components `setup` downloads at runtime with licenses **verified from local artifacts this session**: TabbyAPI **AGPL-3.0** (`LICENSE`), exllamav3 **MIT** (METADATA), torch **BSD-3-Clause** (METADATA). **UNVERIFIED section (must be confirmed at M4 before any release, per director "don't guess"):** `uv` (license file not present locally — confirm from upstream repo when pinning), Python/PSF (uv-managed install — confirm from its LICENSE at provision time), NVIDIA CUDA runtime wheels (EULA — enumerate exact package set from the lock).
...
| M7 | Packaging: tarball, sha256, install.sh, honest README, `THIRD-PARTY.md` finalized (UNVERIFIED entries confirmed per A6) | **S** | M5 | install.sh dry-run in clean dir |
...
- **G14 · A6 licensing (MED, release-blocking).** TabbyAPI **AGPL-3.0** verified — we neither distribute nor modify (reasoning recorded in §6); forking would change that analysis (reinforces Q11). `uv`, Python, NVIDIA wheels **unverified** → `THIRD-PARTY.md` carries an explicit UNVERIFIED section that **must be emptied before any public release** (M7 gate); publishing with guessed licenses is the failure mode this gap exists to prevent.
